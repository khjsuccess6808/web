...
public class ImageUploadServlet extends HttpServlet {
	...
	//	String imageDir = ctx.getRealPath("/up_images"); // /부터는 application
		...
		List<String> imageName=new ArrayList<>();//업로드된 파일명을 저장할 List변수
		...
		try {
			...
			for (FileItem item : list) {
				...
				if (item.isFormField()) {// 일반 요청 파라미터(String)
					String reqValue = item.getString("UTF-8"); // 한글이 있을경우UTF-8"이라고  지정해야한다.
                     //VO에 요청파라미터 set
					...
					if (item.getSize() != 0) { // getSize():long- 업로드된 파일의
						...
						item.write(new File( ctx.getRealPath("/up_images"), fileName));// 매개변수로 받은 파일로
				...
				//이젠  VO로 넣는것이므로 setComment() setUploader() 에 넣고 

			}//end of for문
			vo.setImageNames(imageName); 
			//====> Model호출해서 Business Logic처리 
