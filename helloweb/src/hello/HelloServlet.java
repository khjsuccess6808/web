...
public class HelloServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		String current = String.format("%tY-%<tm-%<td %<tH:%<tM:%<tS", date); // 2017-04-27

		// SimpleDateFormat: java.util.Date <--> String(format())
		// String-->Date(parse())
		// yyyy:년도(4자리) MM:월(2),dd:일(2) HH:시간(24시간-2자),mm:분(2),ss:초(2)

		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String current2 = sdf.format(date);

		// Date dd=sdf.parse("2011-11-20 23:22:05");
		response.setContentType("text/html;charset=UTF-8"); // 공백없이 작성해야함.
